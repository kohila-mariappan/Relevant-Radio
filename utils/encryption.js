...
const createPasswordHashWithSalt = function (plainPassword, passwordHash) {
  const splitValues = passwordHash.split(':')
  const salt = splitValues[1]
  let passwordHashWithSalt = bcrypt.hashSync(plainPassword, salt)
  passwordHashWithSalt = passwordHashWithSalt.substring(29)
  const finalStringToReturn = passwordHashWithSalt + ':' + salt
